...
def test_songzanlin_permission_packet_is_unsent_and_activity_specific() -> None:
    ...
    assert all(
        "conditions_compatible_with_registered_activity" not in x
        and "conditions_review_reference" not in x
        and "conditions_reviewed_by" not in x
        and "conditions_review_date" not in x
        and "conditions_review_rationale" not in x
        for x in out["activity_questions"]
    )
    assert out["response_requirements"][
        "provide_activity_specific_validity_window_and_conditions"
    ] is True
    assert "review_conditions_against_registered_activity" not in out[
