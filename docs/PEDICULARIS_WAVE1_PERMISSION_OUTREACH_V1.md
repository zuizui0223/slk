...
For each A-F row, the draft pre-fills the canonical `registered_activity_definition_reference`, activity-definition SHA-256, registry SHA-256, and hash algorithm. Internal review fields for reviewer, review date, rationale and review reference begin unresolved. They are completed only after the returned condition text is read; the authority-facing inquiry itself does not ask the authority to make the project's compatibility judgment.
