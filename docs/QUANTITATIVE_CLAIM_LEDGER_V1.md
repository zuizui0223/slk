...
| ecological threshold displacement | `MODEL-PREDICTION` | for `Phi(E)=a(E-E_V)`, rare invasion occurs at `E_I=E_V+eta/a` and reverse invasion at `E_R=E_V-eta/a` | exact for the affine environmental slice with locally constant `eta`; otherwise a local first-order prediction |
