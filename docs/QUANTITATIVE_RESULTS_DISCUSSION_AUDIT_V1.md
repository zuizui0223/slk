...
| Ecological context can displace the value and invasion thresholds. | For `Phi(E)=a(E-E_V)`, `E_I=E_V+eta/a` and `E_R=E_V-eta/a`. | Exact only for the registered affine environmental slice with locally constant `eta`; smooth non-affine systems use a local approximation. |
