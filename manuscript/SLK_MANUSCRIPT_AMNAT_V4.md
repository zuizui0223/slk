...
Multifunctional traits can experience genuine conflict without favoring differentiated architecture, and globally favorable differentiation need not become an evolutionary outcome. We develop an architecture-specific estimand hierarchy that separates shared-coordinate conflict, recoverable benefit, architecture cost, local accessibility, invasion, fixation, and long-run occupancy. Conflict is summarized by compromise load `L`; a differentiated comparison recovers benefit `R` and incurs debit `K`, giving the general architecture margin `Phi=R-K`. The region `L>0, Phi<0` represents persistent compromise despite real conflict. Under a quadratic partial-release bridge only, `R=sL`. A single registered composite model yields an exact critical-surface atlas: small-step release switches at `k=k_local`, global endpoint value at `Phi=0` (equivalently `k=k_global`), rare invasion at `Phi=eta`, reverse invasion at `Phi=-eta`, reciprocal fixation at `Phi=0`, and absolute fixation advantage at `3Phi=eta` under weak selection. Connected symmetric rare mutation then places stationary monomorphic occupancy on the same self-play surface as reciprocal fixation. The framework therefore identifies where evolutionary criteria split as new mechanisms enter and where they re-align exactly, while an empirical measurement ladder specifies what additional evidence is required before each stronger evolutionary claim.
...
The main result is therefore a critical-surface transport theorem rather than a claim that every adjacent stage differs. The same endpoint architecture comparison is carried through local release geometry, frequency-dependent competition, finite-population fixation, and rare-mutation occupancy; each new mechanism adds a surface, while the registered fixation-occupancy process forces one exact re-alignment. The downward arrows in Figure 1 are handoffs between estimands, not logical implications.
