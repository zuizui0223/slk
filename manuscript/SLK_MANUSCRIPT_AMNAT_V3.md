...
Multifunctional traits can experience genuine conflict without favoring differentiated architecture, and globally favorable differentiation need not become an evolutionary outcome. We develop an architecture-specific estimand hierarchy that separates shared-coordinate conflict, recoverable benefit, architecture cost, local accessibility, invasion, fixation, and long-run occupancy. Conflict is summarized by compromise load `L`; a differentiated comparison recovers benefit `R` and incurs debit `K`, giving the general architecture margin `Phi=R-K`. The region `L>0, Phi<0` represents persistent compromise despite real conflict. Under a quadratic partial-release bridge only, `R=sL`. Explicit witness regimes show that conflict need not imply positive architecture value, positive value need not imply accessibility, accessible positive value need not imply rare invasion, and rare invasion need not imply reciprocal fixation superiority. Absolute fixation advantage can also disagree with long-run occupancy. Yet under a connected symmetric rare-mutation exponential-Moran process, reciprocal fixation ordering and stationary monomorphic occupancy ordering coincide exactly. The framework therefore predicts both separations and process-level re-alignment, while an empirical measurement ladder specifies what additional evidence is required before each stronger evolutionary claim.
